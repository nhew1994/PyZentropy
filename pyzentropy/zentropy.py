...
def scale_phonon_dos(path, num_atoms=5, plot=False):
    ...
    for volume_per_atom in volumes_per_atom:
        vdos_data_scaled[volume_per_atom] = vdos_data[volume_per_atom][
            vdos_data[volume_per_atom]["Frequency (Hz)"] > 0
        ]
        vdos_data_scaled[volume_per_atom] = vdos_data_scaled[volume_per_atom]
        new_row = pd.DataFrame([[0, 0]], columns=["Frequency (Hz)", "DOS (1/Hz)"])
        vdos_data_scaled[volume_per_atom] = pd.concat(
            [new_row, vdos_data_scaled[volume_per_atom]]
    ...
    for volume_per_atom in volumes_per_atom:
        area = np.trapz(
            vdos_data_scaled[volume_per_atom]["DOS (1/Hz)"],
            vdos_data_scaled[volume_per_atom]["Frequency (Hz)"],
        )
    ...
    if plot == True:
        for volume_per_atom in volumes_per_atom:
            plt.plot(
                vdos_data[volume_per_atom]["Frequency (Hz)"],
                vdos_data[volume_per_atom]["DOS (1/Hz)"],
                label="original",
            )
            plt.plot(
                vdos_data_scaled[volume_per_atom]["Frequency (Hz)"],
